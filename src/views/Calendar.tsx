...
export function Calendar() {
  ...
  const syncAll = async (interactive: boolean) => {
    ...
    try {
      // Always use a dedicated calendar for events. Ensure it exists and store its id.
      let ensuredCalendarId: string | null = settingsRef.current.calendarSubscriptionsCalendarId ?? null
      if (!ensuredCalendarId) {
        try {
          ...
          // If ensuring the dedicated calendar fails, fall back to whatever events have (handled below).
      ...
      for (let idx = 0; idx < list.length; idx++) {
        ...
        try {
          ...
          // Use the dedicated calendar if available, otherwise fall back to the subscription's calendar or primary.
          const targetCalendarId = ensuredCalendarId ?? (s.calendar?.calendarId ?? 'primary')
          ...
          let link: Awaited<ReturnType<typeof upsertRecurringAllDayEvent>>
            ...
            // Bubble up auth errors so UI can prompt the user.
            if (status === 401 || status === 403) throw e

            // For 404 and other non-auth errors, attempt to recreate the event in the ensured calendar,
            // falling back to primary if that fails.
            try {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {state.subscriptions.length > 0 && (
              <button
                type="button"
                className={`inline-flex h-9 w-9 items-center justify-center rounded-md bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-60`}
                ...
                aria-label={t('subscriptions.syncAll') ?? 'Sync all'}
                aria-busy={syncBusy}
              >
                {syncBusy ? (
                  <svg
                ...
                <span className="sr-only">{syncBusy ? (t('subscriptions.syncing') ?? 'Syncing…') : (t('subscriptions.syncAll') ?? 'Sync all')}</span>
              </button>
            )}
