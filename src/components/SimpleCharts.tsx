...
function parseRgb(color: string | null): { r: number; g: number; b: number } | null {
...
function shadeRamp(baseColor: string | null, index: number, total: number, alpha: number) {
  const rgb = parseRgb(baseColor)
  if (!rgb || total <= 1) return rgba(baseColor, alpha)
  const { h, s, l } = rgbToHsl(rgb.r, rgb.g, rgb.b)
  const t = total <= 1 ? 0 : index / (total - 1)
  // Make the first bar noticeably darker and progressively lighten.
  const nextL = clamp(l - 0.18 + t * 0.30, 0.16, 0.82)
  const nextS = clamp(s * (0.92 + t * 0.10), 0.18, 1)
...
export function SimpleBarChart({ items, formatValue, colorClassName = 'text-emerald-500' }: BarChartProps) {
  const base = resolveTailwindTextColor(colorClassName)
  const gridBase = resolveTailwindTextColor('text-slate-200 dark:text-slate-700')
  const tick = resolveTailwindTextColor('text-slate-500 dark:text-slate-400')
  ...
  const data = useMemo(() => {
    const colors = items.map((_, i) => shadeRamp(base, i, items.length, 0.92))
    const hoverColors = items.map((_, i) => shadeRamp(base, i, items.length, 1))
    return {
